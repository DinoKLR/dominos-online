...
const DominoGameMobile: React.FC<DominoGameMobileProps> = ({ onGameEnd, onBackToHome }) => {
  ...
  return (
    <div className="fixed inset-0" style={{ backgroundColor: '#2a802a' }}>
      ...
      {/* Top Header - Compact */}
      <div className="absolute top-0 left-0 right-0 bg-black/80 px-4 py-2 z-20">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <div className="text-xs text-gray-300 font-semibold">YOU</div>
              <div className="text-green-400 text-lg font-bold">{gameState.playerScore}</div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className="text-xs text-gray-300 font-semibold">CPU</div>
              <div className="text-cyan-400 text-lg font-bold">{gameState.computerScore}</div>
            </div>
      ...
      <div className="absolute inset-0 pt-10 pb-16">
        <div className="w-full h-full relative overflow-hidden">
          ...
          >
            <div className="text-center text-green-600/20">
              <div className="text-6xl font-light tracking-wider">NINJA</div>
              <div className="text-6xl font-light tracking-wider">BONES</div>
            </div>
          </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-black/30 backdrop-blur-sm border-t border-green-500/30 px-4 py-2 z-20">
        {/* Top row - Emojis and counts above dominoes */}
        <div className="flex items-center justify-between mb-2">
          {/* Left - Emoji Button */}
          <button className="w-10 h-10 flex items-center justify-center text-2xl hover:scale-110 transition-transform">
            😊
          </button>
          ...
          <div className="flex gap-4 text-sm text-white">
            <div className="text-center">
              <div className="font-extrabold">BOARD</div>
              <div className="text-green-400 font-extrabold text-lg">{gameState.board.length}</div>
            </div>
            <div className="text-center">
              <div className="font-extrabold">BONEYARD</div>
              <div className="text-yellow-400 font-extrabold text-lg">{gameState.boneyard.length}</div>
            </div>
          </div>
